...
class SummaryDialog(QtWidgets.QDialog):
    ...
    def open(self):
        while True:
            file = QtWidgets.QFileDialog.getOpenFileName(self, 'Open SysEx sound file', QtCore.QStandardPaths.writableLocation(QtCore.QStandardPaths.HomeLocation), 'SysEx files (*.syx);;All files (*)')
            if not file: return
